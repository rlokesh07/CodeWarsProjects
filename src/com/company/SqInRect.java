...
public class SqInRect {
    public static List<Integer> sqInRect(int lng, int wdth) {
        ...
        double area = lng * wdth;
        boolean perfect = false;
        int maximumSoution = Math.min(lng, wdth);

        if (lng == wdth){
            perfect = true;
        }
        for (int i = maximumSoution; i > 0;){

            if (i > lng || i > wdth){

            } else {
                if (maximumSoution == lng){
                    wdth = wdth - maximumSoution;

                } else {
                    lng = lng - maximumSoution;

                }

                list.add(i);
                maximumSoution = Math.min(lng, wdth);
            }
